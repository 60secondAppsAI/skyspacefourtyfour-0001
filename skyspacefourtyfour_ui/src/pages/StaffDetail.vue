					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateStaff()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Staffs = true">Add</base-button>
								              <modal :show.sync="modal2Staffs">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Staff Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="StaffId" type="text" placeholder="Enter StaffId" v-model="staffDetails.staffId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">First Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="FirstName" type="text" placeholder="Enter FirstName" v-model="staffDetails.firstName"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Last Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="LastName" type="text" placeholder="Enter LastName" v-model="staffDetails.lastName"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Role</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Role" type="text" placeholder="Enter Role" v-model="staffDetails.role"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateStaff() {
      ...
      const jsonData = JSON.stringify(this.staffDetails);
      const res = await StaffService.update(jsonData);
...
//        this.getAllStaffs();
    ...
    async getStaffDetails() {
      try {
			let response = await StaffService.get(this.staffId);
